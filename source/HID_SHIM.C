...
/*
 * HidRegisterMinidriver(MinidriverRegistration)
 *
 * Called by HID miniport DriverEntry to register with HIDCLASS.
 * XP DDK: NTSTATUS HidRegisterMinidriver(HID_MINIDRIVER_REGISTRATION *)
 * 1 param (__stdcall), ret 4.
 *
 * The HID_MINIDRIVER_REGISTRATION struct contains DriverObject and
 * RegistryPath as fields, so only 1 pointer is passed.
 */
    ...
    HID_MINIDRIVER_REGISTRATION *Registration)
{
    ...
    g_hid_minidriver.DriverObject        = Registration->DriverObject;
    ...
    VxD_Debug_Printf("HID:   DevicesArePolled = ");
