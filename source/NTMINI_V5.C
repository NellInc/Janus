...
/* Embedded miniport drivers.
   NTMINI_USE_SCSI selects which miniport to load:
     ...
     4 = VideoPort (display miniport, uses VIDEO_SHIM)
     5 = Generic ntoskrnl test (uses NTOS_SHIM) */
...
ULONG sp_va_to_pa(ULONG va) {
    ...
    /* 5. Win9x recursive page table (PDE index 0x3FE, not 0x3FF like NT).
       Page directory at VA 0xFFBFE000, page tables at 0xFF800000. */
    {
        ...
        if (*pde & 1) {
            ...
            ULONG pte_val = *pte;
            if (pte_val & 1) {
                pa = (pte_val & 0xFFFFF000) | (va & 0xFFF);
                src = "9xPT";
                goto done;
            }
        }
    ...
    /* 6. Fallback: Win9x kernel VA = 0xC0000000 + PA */
    if (va >= 0xC0000000) {
        pa = va - 0xC0000000;
        src = "KERN";
        goto done;
...
const IMPORT_FUNC_ENTRY scsiport_funcs[] = {
...
#if NTMINI_USE_SCSI == 5
    /* Generic ntoskrnl test: load driver using NTOS_SHIM only. */
    {
        extern int ntos_test_driver(void);
        int nt_rc;
        VxD_Debug_Printf("V5: === NTOS SHIM TEST ===\r\n");
        nt_rc = ntos_test_driver();
        log_hex("V5: ntos_test returned ", (ULONG)nt_rc, "\r\n");
        g_ntmini_initialized = TRUE;
        g_ios_ready = 0;
        VxD_Debug_Printf("V5: NTOS test complete\r\n");
        return 1;
        (void)entry_point; (void)image_base; (void)rc; (void)status; (void)DriverEntry;
    }
#elif NTMINI_USE_SCSI == 4
    /* VideoPort path: register shims now, defer actual test to Sys_VM_Init.
       V86 BIOS calls (Int10) require the System VM, which isn't ready at
       Device_Init time. The test runs from ntmini_deferred_video_test(). */
    {
        extern void video_shim_register(void);
        VxD_Debug_Printf("V5: === VIDEO PORT (deferred to Sys_VM_Init) ===\r\n");
        video_shim_register();
        g_ntmini_initialized = TRUE;
        g_ios_ready = 0;
        VxD_Debug_Printf("V5: VideoPort shims registered, Int10 deferred\r\n");
        return 1;
        (void)entry_point; (void)image_base; (void)rc; (void)status; (void)DriverEntry;
    }
#elif NTMINI_USE_SCSI == 3
    /* NDIS path: register shims now, defer MiniportInitialize to Init_Complete.
       VPICD_Call_When_Hw_Int GPFs at Device_Init (PCI IRQ steering owns the IRQ).
       The actual test runs from _ntmini_deferred_ndis_test() at Init_Complete. */
    {
        extern void ndis_shim_register(void);
        VxD_Debug_Printf("V5: === NDIS (deferred to Init_Complete) ===\r\n");
        ndis_shim_register();
        g_ntmini_initialized = TRUE;
        g_ios_ready = 0;
        VxD_Debug_Printf("V5: NDIS shims registered, init deferred\r\n");
        return 1;
        (void)entry_point; (void)image_base; (void)rc; (void)status; (void)DriverEntry;
    }
...
/* Called from Sys_VM_Init (VXDWRAP). System VM is now ready,
   so V86 BIOS calls (Int10) are safe. No-op for non-VideoPort builds. */
void _ntmini_deferred_video_test(void)
{
#if NTMINI_USE_SCSI == 4
    extern int video_test_miniport(void);
    int vp_rc;
    VxD_Debug_Printf("SVM: === DEFERRED VIDEO PORT TEST ===\r\n");
    vp_rc = video_test_miniport();
    log_hex("SVM: video_test returned ", (ULONG)vp_rc, "\r\n");
    VxD_Debug_Printf("SVM: VideoPort test complete\r\n");
#endif
}

/* Called from Init_Complete (VXDWRAP). VPICD services are ready,
   so we can hook the NIC IRQ before MiniportInitialize enables it. */
void _ntmini_deferred_ndis_test(void)
{
#if NTMINI_USE_SCSI == 3
    ...
    VxD_Debug_Printf("IC: === DEFERRED NDIS TEST ===\r\n");
    ndis_rc = ndis_test_ne2000();
    log_hex("IC: ndis_test returned ", (ULONG)ndis_rc, "\r\n");
    memlog("NDIS_DONE\r\n");
    VxD_Debug_Printf("IC: NDIS test complete\r\n");
#endif
}
