...
/* EthRxIndicateHandler: miniport indicates a received Ethernet frame.
   Prototype: void (EthDB, MACContext, HeaderBuffer, HeaderBufSize,
...
/* Safe no-op for unknown MB function pointer slots.
   Uses zero args so ret (not ret N) regardless of calling convention.
   Logs the return address so we can identify which slot was called. */
...
static void ndis_init_miniport_block(void)
{
    ...
    {
        ULONG off;
        ULONG safe_func = (ULONG)ndis_safe_noop;
        /* Zero entire block */
        for (off = 0; off < NDIS_MINIPORT_BLOCK_SIZE; off += 4) {
            *(ULONG *)(&g_ndis_miniport_block[off]) = 0;
        }
        /* Function pointer region: 0xE0-0x198 inclusive */
        for (off = 0xE0; off <= 0x198; off += 4) {
            *(ULONG *)(&g_ndis_miniport_block[off]) = safe_func;
        }
    }
...
static struct {
    ...
    { "IoBaseAddress",    0x300 },   /* Default NE2000 ISA I/O base */
    { "InterruptNumber",  10    },   /* Default NE2000 ISA IRQ (QEMU) */
    { "BusType",          1     },   /* ISA = 1 */
    { "BusNumber",        0     },
    { "CardType",         0     },   /* 0 = ISA NE2000 (not MCA=1 or PCMCIA=2) */
    { NULL, 0 }
};
 ...
 * For now we just record the interrupt number. Actual VPICD hooking
 * can be added later (start with polling mode).
 */
/*
    ...
    ULONG InterruptMode)
{
    ...
    /* Mask all NE2000 interrupts at the NIC to prevent an interrupt
       storm. Without VPICD hookup, an unhandled ISA IRQ causes a
       triple fault. NE2000 IMR is at iobase+0x0F in page-0. */
    /* Mask this IRQ at the PIC (8259) to prevent unhandled hardware
       interrupts from triple-faulting Win98. Without VPICD hookup,
       the ISA interrupt has no handler. PIC masking is safe — we can
       unmask later when we add VPICD support.
       IRQ 0-7: master PIC port 0x21; IRQ 8-15: slave PIC port 0xA1. */
    {
        ...
        ndis_log_hex("NDIS: Masked IRQ at PIC port=", (ULONG)pic_port, "");
        ndis_log_hex(" mask=", (ULONG)(pic_mask | irq_bit), "\r\n");
    }

    VxD_Debug_Printf("NDIS: IRQ registered (PIC-masked, polling mode)\r\n");
    return NDIS_STATUS_SUCCESS;
}
        ...
        if (g_ndis_miniport.HandleInterruptHandler && g_ndis_miniport.IoPortBase) {
            ...
            for (poll = 0; poll < 500 && !g_rx_captured; poll++) {
                ...
                if (isr_val & 0x0001) {  /* ROK: Receive OK */
                    ...
                    /* Direct RX ring buffer read (bypass HandleInterrupt which
                       crashes due to uninitialized NDIS structures).
                       RTL8139 RX ring: shared memory VA + CAPR offset.
                       Each frame: [status:16][length:16][data...][pad to DWORD] */
                    if (g_ndis_miniport.LastSharedVA) {
